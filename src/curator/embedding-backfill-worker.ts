...
export async function updateRowEmbedding(
  ...
  await pool.query(
    `UPDATE allura_memories SET embedding = $1::ruvector WHERE id = $2`,
    [embeddingStr, id],
  );
}
...
export async function runBackfillCycle(
  ...
  console.log(
    `${LOG_PREFIX} Cycle complete: ${succeeded} embedded, ${failed} failed (of ${rowsFound})`,
  );
...
const isMainModule =
  process.argv[1]?.includes("embedding-backfill-worker.ts");
