...
export class SessionBootstrap {
  ...
  async bootstrap(): Promise<SessionBootstrapResult> {
    ...
    // Step 1: Encoding validation (memory-bank removed — skip)
    // No-op: memory-bank directory has been retired
    ...
    if (this.config.enableCheckpoints && this.checkpointManager) {
      try {
        ...
        await this.checkpointManager.createCheckpoint(this.sessionId, this.config.groupId, state.phase)
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error)
   ...
   */
  async updatePhase(phase: SessionState["phase"], additionalData?: Record<string, unknown>): Promise<void> {
    ...
    if (this.config.enableCheckpoints && this.checkpointManager) {
      await this.checkpointManager.createCheckpoint(this.sessionId, this.config.groupId, phase, additionalData)
    }
