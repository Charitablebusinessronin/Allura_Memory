# Allura Memory → External Agent Integration Plan

> **Status: ACTIVE** — MCP Streamable HTTP transport shipped on `canonical-http-gateway.ts`. GPT Actions path demoted to OPTIONAL/FALLBACK.
> **Updated:** 2026-04-12
> **Original Research Date:** 2026-04-12
> **Architect:** Brooks (Frederick P. Brooks Jr. persona)

## ADR: MCP Streamable HTTP as Primary Integration Path

**Decision:** Add MCP Streamable HTTP transport to the canonical HTTP gateway as the primary integration path for external AI agent runtimes. GPT Actions (REST/OpenAPI bridge) is demoted to OPTIONAL/FALLBACK.

**Context:** The OpenAI Agents JS SDK (`@openai/agents`) ships native MCP support via `MCPServerStreamableHttp` and `hostedMcpTool()`. This eliminates the need for a REST→MCP translation layer. The MCP protocol handles tool discovery automatically — no hand-written OpenAPI schema required.

**Rationale (Brooksian):**
1. **Conceptual Integrity** — The canonical 5-operation interface is preserved. No translation layer. The MCP protocol speaks MCP natively.
2. **Essential vs Accidental Complexity** — The REST bridge, OpenAPI schema, and GPT Actions configuration were accidental complexity. The MCP Streamable HTTP path eliminates all of it.
3. **No Silver Bullet** — This doesn't make AI memory "smarter." It just makes it accessible to any MCP-compatible runtime without a bridge.

**Consequences:**
- `/mcp` endpoint on port 3201 speaks MCP Streamable HTTP natively
- Any TypeScript app using `@openai/agents` can connect via `MCPServerStreamableHttp` or `hostedMcpTool()`
- GPT Actions path remains available but is no longer the primary path
- Zero OpenAPI YAML/JSON files in the codebase for this integration

---

## Implementation (Shipped)

### `canonical-http-gateway.ts` — Dual Transport

The gateway now serves two transports on a single port (3201):

| Route | Transport | Protocol | Use Case |
|-------|-----------|----------|----------|
| `POST/GET/DELETE /mcp` | MCP Streamable HTTP | JSON-RPC over HTTP | **Primary** — OpenAI Agents SDK, any MCP client |
| `GET /tools` | Legacy JSON-RPC | REST | Backward-compatible |
| `POST /tools/call` | Legacy JSON-RPC | REST | Backward-compatible |
| `GET /health` | Health check | REST | Monitoring |

### Auth

- `ALLURA_MCP_AUTH_TOKEN` env var (optional)
- If set: Bearer token required on `/mcp` endpoint
- If unset: Dev mode (no auth)
...
const alluraMemory = new MCPServerStreamableHttp({
...
const agent = new Agent({
  name: 'Allura Agent',
  instructions: 'Use memory tools to store and retrieve knowledge.',
  mcpServers: [alluraMemory],
});
```

Or with hosted MCP (OpenAI Responses API calls the server directly):
...
const agent = new Agent({
  ...
  tools: [
    hostedMcpTool({
      serverLabel: 'allura-memory',
      serverUrl: 'https://api.allura.dev/mcp',
...
- `src/__tests__/mcp-streamable-http.test.ts` — Integration tests for:
  - MCP protocol initialization handshake
  - Tool discovery (all 5 canonical tools)
  - Tool execution round-trip
  - Bearer token auth validation
  - Legacy JSON-RPC backward compatibility
