...
| AD-16 | Ralph model backend is OpenCode | Decided | Ralph drives the OpenCode CLI. Ralph does not talk to any model directly; it invokes OpenCode, checks for completion promise, and loops. What model OpenCode uses (Ollama cloud, local, etc.) is OpenCode's concern, not Ralph's. NOT GitHub Models API — rate limits (15 RPM / 150 req/day) are insufficient for autonomous loops. Previous `github-models` / `GITHUB_TOKEN` references were incorrect and have been corrected. |
