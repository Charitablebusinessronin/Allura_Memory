...
| AD-11 | Human-in-the-loop gating for validation evidence                  | Decided  | Every classic RalphLoop slice requires human review before proceeding. Judge reviews all evidence; no automatic progression to next slice without explicit approval. AD-32 is a scoped exception for Team-Gated Autonomous goal loops, replacing routine human review with Team RAM internal gates and hard-stop escalation.                                                                                                                                                                                                                                                   |
...
| AD-32 | Team-Gated Autonomous Ralph Foundry goal loop | Decided | Goal-level Ralph autonomy uses Team RAM internal gates instead of routine human approval between runs. This is a scoped exception to AD-11. The loop may continue until success or hard stop, but it must declare allowed paths, blocked paths, max runs, validation commands, and stop conditions up front. This preserves autonomy while preventing the second-system effect and runaway patching. See [RALPH-FOUNDRY-AUTO-LOOP.md](./RALPH-FOUNDRY-AUTO-LOOP.md). |
...
| RK-22 | Autonomous goal loop repeats bad strategies or escapes scope | High | Active |
...
| RK-22 | Team-Gated Autonomous goal execution could repeat failed approaches, exceed intended file scope, or mask an architecture problem as an implementation problem. | High | Goal manifests require `allowedPaths`, `blockedPaths`, `maxRuns`, objective `successCriteria`, and hard stop conditions. Judge stops on repeated validation failure and the Brooksian three-failed-fixes rule; Learning writes outcomes to Allura Brain after each run. | Active |
...
- **Ralph Foundry Auto Loop**: [RALPH-FOUNDRY-AUTO-LOOP.md](./RALPH-FOUNDRY-AUTO-LOOP.md) and goal schemas in `json-schema/`
