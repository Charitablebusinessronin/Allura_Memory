...
- In this repository, the default tenant is `allura-system`.
...
```text
allura-brain_memory_search {"query":"recent blockers", "group_id":"allura-system"}
MCP_DOCKER_read_neo4j_cypher {"query":"MATCH (n) RETURN count(n) AS node_count"}
MCP_DOCKER_execute_sql {"sql_query":"SELECT COUNT(*) FROM events WHERE group_id = 'allura-system'"}
```
